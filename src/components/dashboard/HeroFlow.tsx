...
function Orb({ state, activeLoad }: { state: EngineState; activeLoad: number }) {
  ...
  const materialRef = React.useRef<DistortMaterialHandle>(null);
    ...
    <section className="glass-panel relative overflow-hidden">
      ...
      <div className="relative grid grid-cols-1 gap-4 p-5 lg:grid-cols-[minmax(0,1fr)_260px]">
        <div className="relative overflow-hidden rounded-[24px] border border-white/6 bg-black/10">
          <div className="absolute left-6 top-6 z-10 max-w-[31rem]">
            ...
            <p className="mt-3 max-w-[28rem] text-sm leading-6 text-text-muted">
          ...
          <div className="pointer-events-none absolute inset-x-[16%] top-1/2 hidden -translate-y-1/2 lg:block">
            <div className="signal-rail" />
            <span className="signal-pulse signal-pulse-left" />
            <span className="signal-pulse signal-pulse-mid" />
            <span className="signal-pulse signal-pulse-right" />
          </div>

          <div className="grid h-[clamp(320px,44vh,540px)] grid-cols-1 lg:grid-cols-[minmax(240px,0.92fr)_minmax(0,1.35fr)_minmax(240px,0.92fr)]">
...
function PacketCard({ label, detail, tone }: { label: string; detail: string; tone: 'request' | 'healed' }) {
  return (
    <div
      className={`hero-side-card max-w-full rounded-[26px] border px-5 py-5 ${
        tone === 'request'
          ? 'border-accent-error/18 bg-[linear-gradient(180deg,rgba(255,90,118,0.12),rgba(255,90,118,0.05))]'
          : 'border-accent-success/18 bg-[linear-gradient(180deg,rgba(57,217,138,0.14),rgba(57,217,138,0.05))]'
      }`}
    >
      <div className="flex items-center justify-between gap-3">
        <div className="text-[11px] uppercase tracking-[0.3em] text-text-muted">{label}</div>
        <div className={`h-2.5 w-2.5 rounded-full ${tone === 'request' ? 'bg-accent-error shadow-[0_0_18px_rgba(255,90,118,0.35)]' : 'bg-accent-success shadow-[0_0_18px_rgba(57,217,138,0.32)]'}`} />
      </div>
      <div className="mt-4 overflow-hidden break-words text-sm font-medium leading-6 text-white">
        {detail}
      </div>
    </div>
  );
